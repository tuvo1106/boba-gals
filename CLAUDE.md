...
| `docs/api/` | Generated by rswag from the `*_swagger_spec.rb` request specs (ADR-0002, ADR-0030) — **never hand-edit**. `frontend/src/api/generated.ts` is generated from it in turn; `frontend/src/api/types.ts` re-exports that under the names every call site imports. |
| Code comments | Why, not what. Cite `§` sections for design-mandated behavior. |
| `README.md` | Orientation: what this is, the stack, how the pieces fit, where to look, and how to run it. Diagrams of *implemented* behaviour live here; the specification stays in DESIGN.md. |
